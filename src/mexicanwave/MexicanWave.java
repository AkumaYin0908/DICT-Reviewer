...
In many large arenas the crowd is seated in a contiguous circuit all the way around the sport field, and so the wave is able to travel continuously around the arena;
 ...
 the wave can sometimes pass through it. Usually only one wave crest will be present at any given time in an arena, although simultaneous, counter-rotating waves have been produced.
 (Source Wikipedia)
...
public class MexicanWave {
    ...
    public static String[] wave (String str){
        ...
        for(int index=0;index<LENGTH;index++){
            ...
            char upperCase = Character.toUpperCase(str.charAt(index));
            if(index==0){
                        newstr= upperCase + str.substring(index+1);
                    }else{
                        newstr=str.substring(0, index) + upperCase + str.substring(index+1);
                    }
